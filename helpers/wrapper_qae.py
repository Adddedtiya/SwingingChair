...
class WrapperQAE:
    ...
    def train_single_batch(self, image_tensor : torch.Tensor) -> dict[str, float]:
        ...
        combined_tensor_tuple : tuple[torch.Tensor, torch.Tensor] = self.autoencoder.forward_with_loss(image_tensor)

        reconstructed_image : torch.Tensor = combined_tensor_tuple[0]
        entropy_aux_loss    : torch.Tensor = combined_tensor_tuple[1]
        ...
        batch_stats['entropy_loss'] = float(entropy_aux_loss.item())
        batch_stats['psnr_loss']    = float(psnr_loss.item())
        batch_stats['recon_loss']   = float(recon_loss.item())
